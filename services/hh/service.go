...
type Service struct {
	...
	HintedHandoff interface {
		WriteShard(shardID, ownerID uint64, messages []db.Message) error
		Process() error
...
// WriteShard queues the messages write for shardID to node ownerID to handoff queue
func (s *Service) WriteShard(shardID, ownerID uint64, messages []db.Message) error {
	if !s.cfg.Enabled {
		return ErrHintedHandoffDisabled
	}

	return s.HintedHandoff.WriteShard(shardID, ownerID, messages)
}
