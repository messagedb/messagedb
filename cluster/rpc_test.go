...
func TestWriteShardRequestBinary(t *testing.T) {
	...
	sr.AddMessage("cpu", 1.0, time.Unix(0, 0), map[string]string{"host": "serverA"})
	sr.AddMessage("cpu", 2.0, time.Unix(0, 0).Add(time.Hour), nil)
	sr.AddMessage("cpu_load", 3.0, time.Unix(0, 0).Add(time.Hour+time.Second), nil)

	b, err := sr.MarshalBinary()
	if err != nil {
		t.Fatalf("WriteMessagesRequest.MarshalBinary() failed: %v", err)
	}
	if len(b) == 0 {
		t.Fatalf("WriteMessagesRequest.MarshalBinary() returned 0 bytes")
	}

	got := &WriteShardRequest{}
	if err := got.UnmarshalBinary(b); err != nil {
		t.Fatalf("WriteMessagesRequest.UnmarshalMarshalBinary() failed: %v", err)
	}
	...
	if len(got.Messages()) != len(sr.Messages()) {
		t.Errorf("Messages count mismatch: got %v, exp %v", len(got.Messages()), len(sr.Messages()))
	}

	srMessages := sr.Messages()
	gotMessages := got.Messages()
	for i, p := range srMessages {
		g := gotMessages[i]
...
func TestWriteShardResponseBinary(t *testing.T) {
	...
	if err != nil {
		t.Fatalf("WriteMessagesResponse.MarshalBinary() failed: %v", err)
	}
	if len(b) == 0 {
		t.Fatalf("WriteMessagesResponse.MarshalBinary() returned 0 bytes")
	}

	got := &WriteShardResponse{}
	if err := got.UnmarshalBinary(b); err != nil {
		t.Fatalf("WriteMessagesResponse.UnmarshalMarshalBinary() failed: %v", err)
	}
