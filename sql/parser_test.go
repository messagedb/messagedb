...
func TestParser_ParseStatement(t *testing.T) {
	...
	var tests = []struct {
			...
			stmt: &sql.SelectStatement{
				...
				Condition: &sql.BinaryExpr{
					...
					LHS: &sql.BinaryExpr{
						...
						RHS: &sql.StringLiteral{Val: "hosta.influxdb.org"},
					},
					RHS: &sql.BinaryExpr{
						Op:  sql.GT,
		...
		{
			s:    `SHOW SERIES`,
			stmt: &sql.ShowDiscussionsStatement{},
		},
		...
		// SHOW SERIES with OFFSET 0
		{
			...
			stmt: &sql.ShowDiscussionsStatement{Offset: 0},
		},

		// SHOW SERIES with LIMIT 2 OFFSET 0
		{
			s:    `SHOW SERIES LIMIT 2 OFFSET 0`,
			stmt: &sql.ShowDiscussionsStatement{Offset: 0, Limit: 2},
		},
		...
		// DROP SERIES statement
		{
			...
			stmt: &sql.DropDiscussionsStatement{Sources: []sql.Source{&sql.Measurement{Name: "src"}}},
		},
		{
			s: `DROP SERIES WHERE host = 'hosta.influxdb.org'`,
			stmt: &sql.DropDiscussionsStatement{
				Condition: &sql.BinaryExpr{
					Op:  sql.EQ,
					LHS: &sql.VarRef{Val: "host"},
					RHS: &sql.StringLiteral{Val: "hosta.influxdb.org"},
...
// Ensure DropDiscussionsStatement can convert to a string
func TestDropDiscussionsStatement_String(t *testing.T) {
	var tests = []struct {
		s    string
		stmt sql.Statement
	}{
		{
			s:    `DROP DISCUSSIONS FROM src`,
			stmt: &sql.DropDiscussionsStatement{Sources: []sql.Source{&sql.Measurement{Name: "src"}}},
		},
		{
			s: `DROP DISCUSSIONS FROM src WHERE host = 'hosta.influxdb.org'`,
			stmt: &sql.DropDiscussionsStatement{
				Sources: []sql.Source{&sql.Measurement{Name: "src"}},
				Condition: &sql.BinaryExpr{
					Op:  sql.EQ,
		...
		{
			s: `DROP DISCUSSIONS WHERE host = 'hosta.influxdb.org'`,
			stmt: &sql.DropDiscussionsStatement{
				Condition: &sql.BinaryExpr{
					Op:  sql.EQ,
					LHS: &sql.VarRef{Val: "host"},
