...
func TestInspectDataType(t *testing.T) {
	for i, tt := range []struct {
		...
		{float64(100), sql.Float},
	} {
		if typ := sql.InspectDataType(tt.v); tt.typ != typ {
...
func TestSelectStatement_Substatement(t *testing.T) {
	...
	for i, tt := range tests {
		// Parse statement.
		stmt, err := sql.NewParser(strings.NewReader(tt.stmt)).ParseStatement()
		if err != nil {
			t.Fatalf("invalid statement: %q: %s", tt.stmt, err)
		}

		// Extract substatement.
		sub, err := stmt.(*sql.SelectStatement).Substatement(tt.expr)
		if err != nil {
...
func TestSelectStatement_GroupByInterval(t *testing.T) {
	...
	stmt, err := sql.NewParser(strings.NewReader(q)).ParseStatement()
	if err != nil {
		t.Fatalf("invalid statement: %q: %s", stmt, err)
	}

	s := stmt.(*sql.SelectStatement)
...
func TestSelectStatement_SetTimeRange(t *testing.T) {
	...
	s := stmt.(*sql.SelectStatement)
	min, max := sql.TimeRange(s.Condition)
	...
	hasWhere := false
	sql.WalkFunc(s.Condition, func(n sql.Node) {
		if ex, ok := n.(*sql.BinaryExpr); ok {
			if lhs, ok := ex.LHS.(*sql.VarRef); ok {
				if lhs.Val == "foo" {
					if rhs, ok := ex.RHS.(*sql.StringLiteral); ok {
						if rhs.Val == "bar" {
							hasWhere = true
						}
					}
...
func TestSelectStatement_HasWildcard(t *testing.T) {
	...
	for i, tt := range tests {
		...
		if w := stmt.(*sql.SelectStatement).HasWildcard(); tt.wildcard != w {
...
func TestSelectStatement_RewriteWildcards(t *testing.T) {
	var fields = sql.Fields{
		&sql.Field{Expr: &sql.VarRef{Val: "value1"}},
		&sql.Field{Expr: &sql.VarRef{Val: "value2"}},
	}
	var dimensions = sql.Dimensions{
		&sql.Dimension{Expr: &sql.VarRef{Val: "host"}},
		&sql.Dimension{Expr: &sql.VarRef{Val: "region"}},
	}
	...
	for i, tt := range tests {
		...
		// Rewrite statement.
		rw := stmt.(*sql.SelectStatement).RewriteWildcards(fields, dimensions)
		if rw == nil {
...
func TestTimeRange(t *testing.T) {
	for i, tt := range []struct {
		...
		min, max := sql.TimeRange(expr)

		// Compare with expected min/max.
		if min := min.Format(sql.DateTimeFormat); tt.min != min {
			t.Errorf("%d. %s: unexpected min:\n\nexp=%s\n\ngot=%s\n\n", i, tt.expr, tt.min, min)
			continue
		}
		if max := max.Format(sql.DateTimeFormat); tt.max != max {
...
func TestSelectStatement_OnlyTimeDimensions(t *testing.T) {
	...
	for i, tt := range tests {
		...
		if stmt.(*sql.SelectStatement).OnlyTimeDimensions() != tt.exp {
...
func TestRewrite(t *testing.T) {
	...
	act := sql.RewriteFunc(expr, func(n sql.Node) sql.Node {
		switch n := n.(type) {
		case *sql.BinaryExpr:
			return &sql.BinaryExpr{Op: n.Op, LHS: n.RHS, RHS: n.LHS}
		default:
			return n
		}
	})
...
func TestParseString(t *testing.T) {
	...
	for _, tt := range tests {
		...
		_, err = sql.NewParser(strings.NewReader(stmt.String())).ParseStatement()
		if err != nil {
...
func TestEval(t *testing.T) {
	for i, tt := range []struct {
		...
		// Evaluate expression.
		out := sql.Eval(MustParseExpr(tt.in), tt.data)
...
func TestReduce(t *testing.T) {
	...
	for i, tt := range []struct {
		...
		// Fold expression.
		expr := sql.Reduce(MustParseExpr(tt.in), tt.data)
...
// Valuer represents a simple wrapper around a map to implement the sql.Valuer interface.
