...
func TestBackupCommand(t *testing.T) {
	...
	s := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		sw := messagedb.NewSnapshotWriter()
		sw.Snapshot = &messagedb.Snapshot{Files: []messagedb.SnapshotFile{
			{Name: "meta", Size: 5, Index: 10},
		}}
		sw.FileWriters["meta"] = messagedb.NopWriteToCloser(bytes.NewBufferString("55555"))
