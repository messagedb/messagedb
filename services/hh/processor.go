...
func (p *Processor) WriteShard(shardID, ownerID uint64, messages []db.Message) error {
	...
	b := p.marshalWrite(shardID, messages)
...
func (p *Processor) marshalWrite(shardID uint64, messages []db.Message) []byte {
	b := make([]byte, 8)
...
func (p *Processor) unmarshalWrite(b []byte) (uint64, []db.Message, error) {
	ownerID := binary.BigEndian.Uint64(b[:8])
	messages, err := db.ParseMessages(b[8:])
	return ownerID, messages, err
}
