...
func (r *ShardMapper) CreateMapper(sh meta.ShardInfo, stmt string, chunkSize int) (db.Mapper, error) {
	var err error
	var m db.Mapper
	if sh.OwnedBy(r.MetaStore.NodeID()) {
		m, err = r.DataStore.CreateMapper(sh.ID, stmt, chunkSize)
		if err != nil {
			return nil, err
		}
	} else {
