...
func (s *Store) Conversation(database, name string) *Conversation {
	...
	return db.Conversation(name)
}

// deleteConversation loops through the local shards and removes the conversation from each shard
func (s *Store) deleteConversation(name string) error {
	s.mu.RLock()
	defer s.mu.RUnlock()
	for _, sh := range s.shards {
		if err := sh.deleteConversation(name); err != nil {
			return err
		}
	}
	return nil
...
func (s *Store) WriteToShard(shardID uint64, messages []Message) error {
	...
	return sh.WriteMessages(messages)
}
