...
func (*DropDiscussionsStatement) node()        {}
...
func (*ShowDiscussionsStatement) node()        {}
...
func (*DropDiscussionsStatement) stmt()        {}
...
func (*ShowDiscussionsStatement) stmt()        {}
...
type SelectStatement struct {
	...
	// Maxiumum number of discussions to be returned. Unlimited if zero.
	SLimit int

	// Returns discussions starting at an offset from the first one.
	SOffset int
...
(discussionsIDS for xxx.region = 'uswest' union discussionsIDS for yyy.regnion = 'uswest') | discussionsIDS xxx.otherfield = 'XXX'
...
// Substatement returns a single-discussion statement for a given variable reference.
func (s *SelectStatement) Substatement(ref *VarRef) (*SelectStatement, error) {
	...
	// If there is only one discussion source then return it with the whole condition.
	if len(s.Sources) == 1 {
...
// ShowDiscussionsStatement represents a command for listing discussions in the database.
type ShowDiscussionsStatement struct {
	// Conversation(s) the discussions are listed for.
	Sources Sources

	// An expression evaluated on a discussion name or tag.
	Condition Expr
...
// String returns a string representation of the list discussion statement.
func (s *ShowDiscussionsStatement) String() string {
	var buf bytes.Buffer
	_, _ = buf.WriteString("SHOW DISCUSSIONS")
...
// RequiredPrivileges returns the privilege required to execute a ShowDiscussionsStatement.
func (s *ShowDiscussionsStatement) RequiredPrivileges() ExecutionPrivileges {
	return ExecutionPrivileges{{Admin: false, Name: "", Privilege: ReadPrivilege}}
}

// DropDiscussionsStatement represents a command for removing a discussions from the database.
type DropDiscussionsStatement struct {
...
// String returns a string representation of the drop discussions statement.
func (s *DropDiscussionsStatement) String() string {
	var buf bytes.Buffer
	buf.WriteString("DROP DISCUSSIONS")
...
// RequiredPrivileges returns the privilege required to execute a DropDiscussionsStatement.
func (s DropDiscussionsStatement) RequiredPrivileges() ExecutionPrivileges {
	return ExecutionPrivileges{{Admin: false, Name: "", Privilege: WritePrivilege}}
}
