...
import (
	...
	"github.com/messagedb/messagedb/client"
...
func main() {
	...
	fs.BoolVar(&c.ShowVersion, "version", false, "Displays the MessageDB version.")

	// Define our own custom usage to print
	fs.Usage = func() {
		fmt.Println(`Usage of messagedb:
-version
...
# Use messagedb in a non-interactive mode to query the database "metrics" and pretty print json:
$ messagedb -database 'metrics' -execute 'select * from cpu' -format 'json' -pretty

# Dumping out your data:
$ messagedb  -database 'metrics' -dump

# Connect to a specific database on startup and set database context:
$ messagedb -database 'metrics' -host 'localhost' -port '8086'
`)
	}
	...
	if err == nil {
		historyFile = filepath.Join(usr.HomeDir, ".messagedb_history")
...
func showVersion() {
	fmt.Println("MessageDB shell " + version)
}
...
func (c *CommandLine) connect(cmd string) {
	...
	cl, err := client.NewClient(
		client.Config{
			...
			Username:  c.Username,
			Password:  c.Password,
			UserAgent: "MessageDBShell/" + version,
		})
	if err != nil {
...
func (c *CommandLine) help() {
	fmt.Println(`Usage:
        ...
        show organizations    show organizations information
